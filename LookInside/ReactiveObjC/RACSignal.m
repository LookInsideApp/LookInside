...
- (RACSignal *)bind:(RACSignalBindBlock (^)(void))block {
	...
	return [[RACSignal createSignal:^(id<RACSubscriber> subscriber) {
		...
		__block atomic_int_least32_t signalCount;   // indicates self
		atomic_init(&signalCount, 1);

		RACCompoundDisposable *compoundDisposable = [RACCompoundDisposable compoundDisposable];

		void (^completeSignal)(RACDisposable *) = ^(RACDisposable *finishedDisposable) {
			if (atomic_fetch_sub_explicit(&signalCount, 1, memory_order_acq_rel) == 1) {
		...
		void (^addSignal)(RACSignal *) = ^(RACSignal *signal) {
			atomic_fetch_add_explicit(&signalCount, 1, memory_order_acq_rel);
