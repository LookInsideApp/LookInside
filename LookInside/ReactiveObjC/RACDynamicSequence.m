...
- (void)dealloc {
	static atomic_int_least32_t directDeallocCount = 0;

	if (atomic_fetch_add_explicit(&directDeallocCount, 1, memory_order_relaxed) + 1 >= DEALLOC_OVERFLOW_GUARD) {
		atomic_fetch_sub_explicit(&directDeallocCount, DEALLOC_OVERFLOW_GUARD, memory_order_relaxed);
