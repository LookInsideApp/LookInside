...
- (RACDisposable *)connect {
	int32_t expected = 0;
	BOOL shouldConnect = atomic_compare_exchange_strong_explicit(&_hasConnected, &expected, 1, memory_order_acq_rel, memory_order_acquire);
...
- (RACSignal *)autoconnect {
	__block atomic_int_least32_t subscriberCount;
	atomic_init(&subscriberCount, 0);

	return [[RACSignal
		createSignal:^(id<RACSubscriber> subscriber) {
			atomic_fetch_add_explicit(&subscriberCount, 1, memory_order_acq_rel);
			...
			return [RACDisposable disposableWithBlock:^{
				...
				if (atomic_fetch_sub_explicit(&subscriberCount, 1, memory_order_acq_rel) == 1) {
