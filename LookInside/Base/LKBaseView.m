...
static void *LKBaseViewEffectiveAppearanceContext = &LKBaseViewEffectiveAppearanceContext;
...
@property(nonatomic, assign) BOOL isObservingEffectiveAppearance;
...
- (instancetype)initWithFrame:(NSRect)frameRect {
    if (self = [super initWithFrame:frameRect]) {
        ...
        [self _beginObservingAppearanceIfNeeded];
...
- (void)dealloc {
    if (self.isObservingEffectiveAppearance) {
        [self removeObserver:self forKeyPath:@"effectiveAppearance" context:LKBaseViewEffectiveAppearanceContext];
    }
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context {
    if (context == LKBaseViewEffectiveAppearanceContext) {
...
- (void)_beginObservingAppearanceIfNeeded {
    if (self.isObservingEffectiveAppearance) {
        return;
    }
    [self addObserver:self forKeyPath:@"effectiveAppearance" options:NSKeyValueObservingOptionInitial | NSKeyValueObservingOptionNew context:LKBaseViewEffectiveAppearanceContext];
    self.isObservingEffectiveAppearance = YES;
}
