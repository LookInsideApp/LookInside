...
@interface RACCommand () {
	...
	atomic_uint_least32_t _allowsConcurrentExecution;
}
...
- (BOOL)allowsConcurrentExecution {
	return atomic_load_explicit(&_allowsConcurrentExecution, memory_order_acquire) != 0;
}

- (void)setAllowsConcurrentExecution:(BOOL)allowed {
	atomic_store_explicit(&_allowsConcurrentExecution, allowed ? 1u : 0u, memory_order_release);
	[self.allowsConcurrentExecutionSubject sendNext:@(atomic_load_explicit(&_allowsConcurrentExecution, memory_order_acquire) != 0)];
}
