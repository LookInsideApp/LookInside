...
- (RACDisposable *)setKeyPath:(NSString *)keyPath onObject:(NSObject *)object nilValue:(id)nilValue {
	...
	RACDisposable *subscriptionDisposable = [self subscribeNext:^(id x) {
		...
		void *objectPointer = atomic_load_explicit(&objectPtr, memory_order_acquire);
		__strong NSObject *object __attribute__((objc_precise_lifetime)) = (__bridge __strong id)objectPointer;
	...
	RACDisposable *clearPointerDisposable = [RACDisposable disposableWithBlock:^{
		...
		atomic_exchange_explicit(&objectPtr, NULL, memory_order_acq_rel);
	}];
...
- (RACSignal *)deliverOnMainThread {
	return [[RACSignal createSignal:^(id<RACSubscriber> subscriber) {
		__block atomic_int_least32_t queueLength;
		atomic_init(&queueLength, 0);
		
		void (^performOnMainThread)(dispatch_block_t) = ^(dispatch_block_t block) {
			int32_t queued = atomic_fetch_add_explicit(&queueLength, 1, memory_order_acq_rel) + 1;
			if (NSThread.isMainThread && queued == 1) {
				block();
				atomic_fetch_sub_explicit(&queueLength, 1, memory_order_acq_rel);
			} else {
