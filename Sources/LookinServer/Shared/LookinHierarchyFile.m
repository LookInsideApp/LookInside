...
+ (NSError *)verifyHierarchyFile:(LookinHierarchyFile *)hierarchyFile {
    ...
    if (hierarchyFile.serverVersion < LOOKIN_SUPPORTED_SERVER_MIN) {
        ...
        NSString *detail = [NSString stringWithFormat:NSLocalizedString(@"The document was created by a LookInside app with too old version. Current LookInside app version is %@, but the document version is %@.", nil), @(LOOKIN_CLIENT_VERSION), @(fileVersion)];
    ...
    if (hierarchyFile.serverVersion > LOOKIN_SUPPORTED_SERVER_MAX) {
        // 文件版本太新
        NSString *detail = [NSString stringWithFormat:NSLocalizedString(@"Current LookInside app is too old to open this document. Current LookInside app version is %@, but the document version is %@.", nil), @(LOOKIN_CLIENT_VERSION), @(hierarchyFile.serverVersion)];
