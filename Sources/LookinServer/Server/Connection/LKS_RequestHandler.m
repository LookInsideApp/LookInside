...
- (void)_handleInvokeWithObject:(NSObject *)obj selector:(SEL)selector resultDescription:(NSString **)description resultObject:(LookinObject **)resultObject error:(NSError **)error {
    ...
    if (signature.numberOfArguments > 2) {
        *error = LookinErrorMake(LKS_Localized(@"LookInside doesn't support invoking methods with arguments yet."), @"");
        return;
    }
    ...
    if (strcmp(returnType, @encode(void)) == 0) {
        ...
        *description = NSStringFromUIOffset(targetValue);
        ...
        if ([argType_string hasPrefix:@"@"] || [argType_string hasPrefix:@"^{"]) {
            ...
            *description = [NSString stringWithFormat:LKS_Localized(@"%@ was invoked successfully, but LookInside can't parse the return value:%@"), NSStringFromSelector(selector), argType_string];
        }
    }
}
